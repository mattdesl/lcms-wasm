...
Module.cmsDoTransformView = cmsDoTransformView;

function cmsDoTransform(transform, inputArr, size) {
  return cmsDoTransformView(transform, inputArr, size).slice();
}

function cmsDoTransformView(transform, inputArr, size) {
  ...
  var outputArr;
  if (outputIsFloat) {
    outputArr = new Float32Array(
      Module.HEAPU8.buffer,
      outputBuffer,
      outputLength
    ).slice();
  } else {
    outputArr = new Uint8Array(
      Module.HEAPU8.buffer,
      outputBuffer,
      outputLength
    ).slice();
